...
#   of a given year. Includes temperature data. Normalized.
#   Some sites lack temperature data.
# @param:
#--------------------------------------------------------------#
plot.hourly_mean.pm_temp <- function(df, years = years.all, seasons = seasons.all) {
  df <- df %>%
    subset(subset = Year.Local %in% years &
             Season.Local %in% seasons)
  
  temperature.df <- subset(all.temperature, subset = County.Name %in% unique(df$County.Name) &
                             State.Name %in% unique(df$State.Name) &
                             Year.Local %in% unique(df$Year.Local) &
                             Season.Local %in% unique(df$Season.Local))
  
  # Aggregate data
  temperature.ag <- aggregate(Sample.Measurement ~ Time.Local+Season.Local+Year.Local,
                              temperature.df, mean)
  ag <- aggregate(Sample.Measurement ~ Time.Local+Season.Local+Year.Local,
                  df, mean)
  
  # Normalize data
  temperature.ag$Sample.Measurement <- as.numeric(scale(temperature.ag$Sample.Measurement))
  ag$Sample.Measurement <- as.numeric(scale(ag$Sample.Measurement))
...
plot.hourly_mean.pm <- function(df, years = years.all, seasons = seasons.all) {
  ...
  ag %>%
    ...
    labs(x = "Hour", y = "PM2.5 Concentration (Micrograms/cubic meter)", color = ("Site")) +
    scale_x_continuous(breaks = c(0, 6, 12, 18, 23),
                       label = c("00:00", "06:00", "Noon", "18:00", "23:00")) +
    ggtitle(paste0("PM2.5 FRM - Aggregated Hourly data - ", df$County.Name, ", ", df$State.Name),
            subtitle = paste0(unique(years), collapse = ", ")) +
    theme_bw()
}
...
# @param:
#--------------------------------------------------------------#
plot.range.mean_peak.pm <- function(df, years = years.all, seasons = seasons.all) {
  df <- df %>%
    subset(subset = Year.Local %in% years &
             Season.Local %in% seasons)
  ...
  ag %>%
    ggplot() +
    geom_point(aes(x = Date.Local, y = Sample.Measurement, color = Type, shape = Type), 
...
# @param: 
#--------------------------------------------------------------#
plot.environment.hourly_mean.pm <- function(df = all.pm, seasons = seasons.all, years = years.all, state) {
  df <- df %>%
...
plot.environment.site_count.pm <- function(df = all.pm, seasons = seasons.all, years = years.all) {
  ...
  df %>%
    ...
    geom_bar() +
    labs(x = "Location Environment", y = "Count") +
