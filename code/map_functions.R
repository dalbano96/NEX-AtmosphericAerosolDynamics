...
load.map <- function() {
  leaflet(unique(select(all.pm, c(Longitude, Latitude, Site.Num, County.Name)))) %>%
    ...
    # Temperature Sites
    # addCircles(data = unique(select(all.temperature, c(Longitude, Latitude, Site.Num, County.Name, State.Name))),
    #            lng = ~Longitude,
    #            lat = ~Latitude,
    #            color = "green",
    #            radius = 500,
    #            label = ~paste("[Temperature] Site Num: ", Site.Num, ", ",
    #                           "County Name: ", County.Name, ", ",
    #                           "State Name: ", State.Name)) %>%
    # EPA AQS Sites
    addCircles(data = unique(select(all.pm, c(Longitude, Latitude, Site.Num, County.Name, State.Name, Location.Setting))), 
               lng = ~Longitude, 
               lat = ~Latitude,
               color = "blue",
               radius = 250,
...
#                 # This is where the magnitude range is selected
#                 sliderInput("range", "POC", min(all.pm$POC), max(all.pm$POC),
#                             value = range(all.pm$POC), step = 0.1
#                 ),
...
#   # })
#   
#   filtered.pm_data <- reactive({
#     unique(select(all.pm, c(Latitude, Longitude)))
#   })
#   
#   # This reactive expression represents the palette function,
#   # which changes as the user makes selections in UI.
#   colorpal <- reactive({
#     colorNumeric(input$colors, filtered.pm_data$POC)
#   })
#   
...
#     leaflet(filtered.pm_data) %>% addTiles() %>%
#       fitBounds(~min(Longitude), ~min(Latitude), ~max(Longitude), ~max(Latitude))
#   })
#   
...
#     pal <- colorpal()
#     
#     leafletProxy("map", data = filtered.pm_data()) %>%
#       clearShapes() %>%
...
#   observe({
#     proxy <- leafletProxy("map", data = filtered.pm_data)
#     
